...
if($asunto == 2){ // registro
    $titulo = '¡TÚ PEDIDO SE HA REALIZADO CON ÉXITO!';
} 
if($asunto == 3){
...
//Tell PHPMailer to use SMTP
    ...
    if($plantilla == 5){
        $enlace = Encrypt::Encrypt($destinatario, md5(TD_SERVER));
        $mail->Body = self::Recovery($nombre_destinatario, $enlace);
    }
...
if($asunto == 2){ // registro
     $mesaje = $nombre_destinatario . " En los próximos minutos personal de atención al cliente de Justo Minimarket se comunicará contigo para coordinar detalles de tu orden y su despacho<br><br>
     Cuando el mensajero te entregue tu orden, deberás pagarle a él, el monto acordado";
} 
if($asunto == 3){
...
static public function Recovery($nombre_destinatario, $enlace){

    $mesaje = $nombre_destinatario . " para recuperar su password ingrese en el siguiente enlace, y ingresa un nuevo password <br><br>Si ud no solicitó un cambio de password ingore este mensaje";
            ...
            <td valign="middle" style="border: none; margin: 0px; padding: 0px;">
                ...
                <table cellspacing="0" cellpadding="0" border="0" width="100%" style="margin: 0px; padding: 0px">
                    <tbody>
                        ...
                        <h3><a href="https://justomarket.com/?recoverypass&u='.$enlace.'&hash='.md5($enlace).'&t='.Fechas::Format(date("H:i:s")).'" target="_blank">RECUPERAR PASSWORD</a></h3>
...
Este mensaje fué enviado el '. Fechas::FechaEscrita(date("d-m-Y")) .' a las '. date("H:i:s") .'. Este enlace tiene validéz de una hora 
